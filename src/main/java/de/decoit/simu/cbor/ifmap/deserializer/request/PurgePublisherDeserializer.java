...
/**
 * The singleton instance of this class may be used to deserialize requests of type {@link CBORPurgePublisherRequest}.
 * 
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class PurgePublisherDeserializer implements InternalRequestDeserializer<CBORPurgePublisherRequest> {
	private static PurgePublisherDeserializer instance;
	 ...
	 */
	public static PurgePublisherDeserializer getInstance() {
		if(instance == null) {
			instance = new PurgePublisherDeserializer();
		}
	 ...
	 */
	private PurgePublisherDeserializer() {}
	
	
	@Override
	public CBORPurgePublisherRequest deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case SessionAttributeGroup.SESSION_ID:
					...
					break;
				case CBORPurgePublisherRequest.IFMAP_PUBLISHER_ID:
					publisherId = RequestDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
			}
		}
		
		CBORPurgePublisherRequest rv = new CBORPurgePublisherRequest(sessionId, publisherId);
