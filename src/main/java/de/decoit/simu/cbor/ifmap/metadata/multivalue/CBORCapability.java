...
/**
 * Java representation of the IF-MAP capability metadata.
 *
...
@Slf4j
public class CBORCapability extends AbstractMultiValueMetadata {
	public static final String XML_NAME = "capability";
	...
	/**
	 * Create a new capability metadata.
	 ...
	 */
	public CBORCapability(String ifMapPublisherId, ZonedDateTime ifMapTimestamp, String name) {
	 ...
	 * @param name Capability name
	 */
	public CBORCapability(String name) {
	...
	protected void serializeNestedElements(final ArrayBuilder<?> builder) throws CBORSerializationException {
		try {
			...
			// Serialize 'name'
			{
				DataItem cborName = this.getNestedElementNameMapping(CBORCapability.NAME, elementEntry);
			...
			if(this.administrativeDomain != null) {
				DataItem cborName = this.getNestedElementNameMapping(CBORCapability.ADMINISTRATIVE_DOMAIN, elementEntry);
