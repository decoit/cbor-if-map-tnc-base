...
/**
 * The singleton instance of this class may be used to deserialize metadata of type {@link CBORDeviceAttribute}.
 * 
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class DeviceAttributeDeserializer implements VendorMetadataDeserializer<CBORDeviceAttribute> {
	private static DeviceAttributeDeserializer instance;
	 ...
	 */
	public static DeviceAttributeDeserializer getInstance() {
		if(instance == null) {
			instance = new DeviceAttributeDeserializer();
		}
	 ...
	 */
	private DeviceAttributeDeserializer() {}
	
	
	@Override
	public CBORDeviceAttribute deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case CBORDeviceAttribute.IFMAP_PUBLISHER_ID:
					publisherId = MetadataDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
				case CBORDeviceAttribute.IFMAP_TIMESTAMP:
					timestampDi = attrValue;
					break;
				case CBORDeviceAttribute.IFMAP_TIMESTAMP_FRACTION:
		...
		for(int i=0; i<nestedTagsDataItems.size(); i=i+4) {
			...
			// The attributes array is ignore because a IF-MAP 'device-attribute' has no attributes on nested tags
			...
			if(!MetadataDeserializerManager.isSimpleValueNull(ntNamespace)) {
				throw new CBORDeserializationException("Unexpected nested element with namespace found inside 'device-attribute' element");
			}
		...
		CBORDeviceAttribute rv;
		if(publisherId != null && timestamp != null) {
			rv = new CBORDeviceAttribute(publisherId, timestamp, name);
		}
		else {
			rv = new CBORDeviceAttribute(name);
		}
